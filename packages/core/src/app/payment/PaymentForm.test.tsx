...
describe('PaymentForm', () => {
  ...
  it('does not render a manual loan amount field if loan amount can be applied', () => {
    render(<PaymentFormTest {...defaultProps} usableStoreCredit={100} />);

    expect(
      screen.queryByRole('checkbox', {
        name: 'Apply $112.00 loan amount to order',
      }),
    ).not.toBeInTheDocument();
    expect(screen.queryByText(/loan amount to order/)).not.toBeInTheDocument();
  });

  it('does not render loan amount field if loan amount cannot be applied', () => {
    render(<PaymentFormTest {...defaultProps} />);
  ...
  it('shows overlay if store credit can cover total cost of order', () => {
    ...
    expect(
      screen.queryByRole('checkbox', {
        name: 'Apply $1,120,000.00 loan amount to order',
      }),
    ).not.toBeInTheDocument();
    expect(
  ...
  it('does not show overlay if store credit cannot cover total cost of order', () => {
    ...
    expect(
      screen.queryByRole('checkbox', {
        name: 'Apply $1.12 loan amount to order',
      }),
    ).not.toBeInTheDocument();
    expect(
